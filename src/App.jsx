...
function FolderForm({ initial, onSave, onCancel }) {
  ...
  useEffect(() => {
    setF(initial || { title:"", amount:"", note:"", color:FOLDER_COLORS[0] });
  }, [initial]);
  const s = (k,v) => setF(p=>({...p,[k]:v}));
  // ✅ amount is now optional — only title is required
  const ok = f.title.trim();
  return (
    <div className="form-grid">
      ...
      <div className="form-field">
        <label>Total Amount (₹) <span style={{fontWeight:400,textTransform:"none"}}>(optional)</span></label>
        <input type="number" min="0" value={f.amount} onChange={e=>s("amount",e.target.value)} placeholder="Leave empty if no budget limit" />
      </div>
      <div className="form-field">
      ...
      <div className="form-row-2">
        ...
        <button className="btn-save" disabled={!ok} onClick={()=>ok&&onSave({...f,amount:parseFloat(f.amount)||0})}>
...
function TxnForm({ initial, folders, defaultFolderId, onSave, onCancel }) {
  ...
  useEffect(() => {
    if (!initial) {
      setF({ type:"expense", amount:"", date:today(), description:"", category:"Food", notes:"", folderId: defaultFolderId || "" });
    } else {
      setF(initial);
    }
  }, [initial, defaultFolderId]);
  const s = (k,v) => setF(p=>({...p,[k]:v}));
    ...
    <div className="form-grid">
      ...
      <div className="form-field">
        ...
        <select value={f.folderId||""} onChange={e=>s("folderId",e.target.value||"")}>
          ...
          {folders.map(fl=><option key={fl.id} value={fl.id}>📁 {fl.title}{fl.amount>0?` · ${fmt(fl.amount)}`:""}</option>)}
        </select>
      </div>
      <div className="form-field full">
...
function FolderCard({ folder, txns, onOpen, onEdit, onDelete }) {
  ...
  const hasLimit = folder.amount > 0;
  const pct    = hasLimit ? Math.min((spent/folder.amount)*100,100) : 0;
  const over   = hasLimit && spent > folder.amount;
  ...
  return (
    <div className="folder-card" onClick={onOpen} style={{borderTop:`3px solid ${folder.color}`}}>
      ...
      {hasLimit
        ...
        : <div className="fc-amount" style={{color:folder.color,fontSize:14,fontWeight:600}}>No limit set</div>
      }
      ...
      {!hasLimit && <div className="fc-stats"><span className="fc-spent">Spent: {fmt(spent)}</span></div>}
    ...
    <div className="page">
      ...
      <div className="fd-hero" style={{borderLeft:`4px solid ${folder.color}`}}>
        <div className="fd-row">
          {hasLimit && <div className="fdh-stat"><span>Folder Amount</span><strong style={{color:folder.color}}>{fmt(folder.amount)}</strong></div>}
          ...
          {hasLimit && <div className="fdh-stat"><span>{over?"Over By":"Remaining"}</span><strong style={{color:over?"#ef4444":"#3b82f6"}}>{fmt(Math.abs(folder.amount-spent))}</strong></div>}
          {!hasLimit && <div className="fdh-stat"><span>Transactions</span><strong style={{color:"#8b5cf6"}}>{ftxns.length}</strong></div>}
        </div>
        {hasLimit && (
          <>
...
export default function App() {
  ...
  useEffect(() => { localStorage.setItem("kp_txns", JSON.stringify(txns)); }, [txns]);
  useEffect(() => { localStorage.setItem("kp_folders", JSON.stringify(folders)); }, [folders]);
  useEffect(() => { localStorage.setItem("kp_budgets", JSON.stringify(budgets)); }, [budgets]);
  useEffect(() => { localStorage.setItem("kp_dark", JSON.stringify(dark)); }, [dark]);
  ...
  const [addFolderCtx, setAddFolderCtx]   = useState("");
  ...
  const T = {
    bg:dark?"#0f1117":"#f0f2f8", surface:dark?"#1a1d27":"#ffffff",
    surf2:dark?"#21253a":"#f7f8fc", border:dark?"#2a2f45":"#e2e6f0",
    text:dark?"#e8eaf6":"#1a1d2e", muted:dark?"#6b7280":"#8892a4", accent:"#6366f1"
  };
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",height:"100dvh",background:T.bg}}>
      ...
      <div className="mobile-topbar">
        ...
        <button onClick={()=>setDark(d=>!d)} style={{background:"none",border:"none",cursor:"pointer",fontSize:22,padding:6,color:T.muted}}>{dark?"☀️":"🌙"}</button>
      </div>

      <div className="app">
        <nav className="sidebar">
          <div className="brand"><span className="bi">💹</span><span className="bn">Kharcha<span>Pro</span></span></div>
                    ...
                    <div className="card">
                      ...
                      <div className="fp-grid">
                        {folders.map(fl=>{
                          ...
                          const hasLim=fl.amount>0;
                          const p=hasLim?Math.min((sp/fl.amount)*100,100):0;
                          const ov=hasLim&&sp>fl.amount;
                          return (
                            <div key={fl.id} className="fp-item" onClick={()=>{setPage("folders");setOpenFolder(fl.id);}}>
                              <div className="fp-head">
                                ...
                                {hasLim
                                  ...
                                  : <span style={{fontSize:11,color:fl.color,fontWeight:600}}>{fmt(sp)} spent</span>
                                }
                              </div>
                              <div className="fp-sub">{hasLim?`${fmt(sp)} of ${fmt(fl.amount)}`:`${txns.filter(t=>t.folderId===fl.id).length} txns`}</div>
                              {hasLim&&<div className="fp-bar-bg"><div className="fp-bar" style={{width:p+"%",background:ov?"#ef4444":fl.color}}/></div>}
                            </div>
                          );
                        })}
                      </div>
            ...
            <div className="page">
              ...
              {folders.length===0 ? (
                <div className="empty-state">
                  ...
                  <p>Folders help you group transactions by purpose. Give it a title and optionally a budget amount to track spending against it.</p>
                ...
                <div className="folder-grid">
                  ...
                  <div className="folder-add-card" onClick={()=>setShowNewFolder(true)}>
                    <span className="fac-plus">＋</span><span>New Folder</span>
                  </div>
                </div>
              )}
            </div>
                          ...
                          {folders.map(fl=>{
                            ...
                            return (
                              <div key={fl.id} className="bt-row" style={{cursor:"pointer"}} onClick={()=>{setPage("folders");setOpenFolder(fl.id);}}>
                                ...
                                <span style={{color:fl.color}}>{hasLim?fmt(fl.amount):"—"}</span>
                                <span style={{color:"#ef4444"}}>{fmt(sp)}</span>
                                <span>{hasLim?<span className={`status-tag${ok?" ok":" over"}`}>{ok?`${fmt(fl.amount-sp)} left`:`Over ${fmt(sp-fl.amount)}`}</span>:<span style={{opacity:.5,fontSize:11}}>No limit</span>}</span>
                              </div>
                            );
                          })}
                        </div>
                ...
                <div className="card report-card">
                  ...
                  {folders.length===0 ? <p style={{fontSize:13}}>No folders created yet.</p>
                    : folders.slice(0,5).map(fl=>{ const sp=txns.filter(t=>t.folderId===fl.id&&t.type==="expense").reduce((s,t)=>s+t.amount,0); return (
                        <div key={fl.id} className="rep-cat">
                          <span style={{color:fl.color}}>📁 {fl.title}</span>
                          <span>{fmt(sp)}{fl.amount>0?` / ${fmt(fl.amount)}`:""}</span>
                        </div>
                      );})
                  }
                </div>
      ...
      <nav className="bottom-nav">
        {[["dashboard","🏠","Home"],["folders","📁","Folders"],["transactions","📋","Txns"],["analytics","📊","Charts"],["settings","⚙️","More"]].map(([id,ic,lb])=>(
          <button key={id} className={`bn-btn${page===id?" active":""}`} onClick={()=>goPage(id)}>
            <span className="bn-icon">{ic}</span><span>{lb}</span>
      ...
      <Modal open={showNewFolder} onClose={()=>setShowNewFolder(false)} title="New Folder">
        {showNewFolder&&<FolderForm key="new-folder" onSave={saveFolder} onCancel={()=>setShowNewFolder(false)}/>}
      </Modal>
      <Modal open={!!editFolder} onClose={()=>setEditFolder(null)} title="Edit Folder">
        {editFolder&&<FolderForm key={editFolder.id} initial={editFolder} onSave={saveFolder} onCancel={()=>setEditFolder(null)}/>}
      </Modal>
...
.bi{font-size:24px}.bn{font-family:'Syne',sans-serif;font-size:18px;font-weight:800;color:${text}}.bn span{color:${accent}}
...
.nav-btn:hover{background:${surf2};color:${text}}.nav-btn.active{background:${accent}20;color:${accent};font-weight:600}
.nav-label{flex:1}.nav-badge{background:${accent};color:#fff;font-size:10px;font-weight:700;padding:1px 6px;border-radius:99px}
...
.stat-value{font-size:19px;font-weight:700;margin:2px 0}.stat-sub{font-size:11px;color:${muted}}
...
.es-icon{font-size:54px}.empty-state h3{font-family:'Syne',sans-serif;font-size:20px}
...
.fc-spent{color:${muted}}.fc-meta{font-size:11px;color:${muted}}
...
.folder-add-card:hover{border-color:${accent};color:${accent}}.fac-plus{font-size:28px;font-weight:300}
...
.fd-bar{height:100%;border-radius:4px;transition:width .5s}.fd-pct{font-size:12px;color:${muted}}
...
.txn-row-full:last-child{border:none}.txn-row-full:active{background:${surf2}}.txn-row-full .txn-actions{opacity:1}
.cat-icon{font-size:20px;flex-shrink:0}.cat-icon.big{font-size:23px}
...
.txn-amt.income,.income{color:#22c55e}.txn-amt.expense,.expense{color:#ef4444}
...
.search-input{flex:1;min-width:160px}.txn-count{font-size:12px;color:${muted}}
.top-cats{display:flex;flex-direction:column;gap:11px}
.top-cat-row{display:flex;align-items:center;gap:10px}
.rank{font-size:12px;color:${muted};width:20px;font-weight:600}.cat-bar-wrap{flex:1}
.cat-bar-label{display:flex;justify-content:space-between;font-size:13px;font-weight:500;margin-bottom:4px}
.cat-bar-bg{height:6px;background:${border};border-radius:3px;overflow:hidden}.cat-bar-fill{height:100%;border-radius:3px}
...
.ldot{width:8px;height:8px;border-radius:50%;flex-shrink:0}.lval{margin-left:auto;color:${muted}}
...
.bt-row:last-child{border:none}.bt-row:active{background:${surf2}}
.status-tag{padding:2px 7px;border-radius:20px;font-size:11px;font-weight:600}
.status-tag.ok{background:#22c55e22;color:#22c55e}.status-tag.over{background:#ef444422;color:#ef4444}
.report-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:14px}
.report-card{text-align:center}.report-icon{font-size:40px;margin-bottom:10px}
.report-card h3{font-size:15px;margin-bottom:7px}.report-card p{font-size:12.5px;color:${muted};margin-bottom:14px}
...
.report-stats span{font-size:11px;color:${muted};text-transform:uppercase}.report-stats strong{font-size:15px;font-weight:700}
...
.budget-item{display:flex;flex-direction:column;gap:5px}.budget-item label{font-size:13px;font-weight:500}
...
.color-dot:active{transform:scale(1.2)}.color-dot.sel{border-color:#fff;box-shadow:0 0 0 2px ${accent};transform:scale(1.1)}
...
.bottom-nav{display:none}.mobile-topbar{display:none}.mobile-fab{display:none}
@media(max-width:900px){
  .stat-grid{grid-template-columns:1fr 1fr}.dash-grid{grid-template-columns:1fr}
  .report-grid{grid-template-columns:1fr}.budget-grid{grid-template-columns:1fr 1fr}
  .fd-row{grid-template-columns:1fr 1fr}
}
@media(max-width:640px){
  .sidebar{display:none}
  .mobile-topbar{display:flex;align-items:center;justify-content:space-between;padding:12px 16px;padding-top:calc(12px + env(safe-area-inset-top));background:${surface};border-bottom:1px solid ${border};position:sticky;top:0;z-index:50;flex-shrink:0}
  .bottom-nav{display:flex;position:fixed;bottom:0;left:0;right:0;height:calc(62px + env(safe-area-inset-bottom));padding-bottom:env(safe-area-inset-bottom);background:${surface};border-top:1px solid ${border};z-index:100;align-items:stretch}
  .bn-btn{flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:3px;border:none;background:transparent;color:${muted};cursor:pointer;font-size:10px;font-family:inherit;font-weight:500;padding:6px 2px;transition:color .14s;position:relative}
  .bn-btn.active{color:${accent}}.bn-btn .bn-icon{font-size:20px;line-height:1}
  .bn-btn .bn-badge{position:absolute;top:6px;right:calc(50% - 18px);background:${accent};color:#fff;font-size:9px;font-weight:700;padding:1px 5px;border-radius:99px;min-width:16px;text-align:center}
  .mobile-fab{position:fixed;bottom:calc(70px + env(safe-area-inset-bottom));right:18px;width:54px;height:54px;background:${accent};color:#fff;border:none;border-radius:50%;font-size:26px;cursor:pointer;box-shadow:0 4px 18px rgba(99,102,241,.5);z-index:99;display:flex;align-items:center;justify-content:center}
  .main{padding:16px 14px;padding-bottom:calc(80px + env(safe-area-inset-bottom))}
  .stat-grid{grid-template-columns:1fr 1fr}.folder-grid{grid-template-columns:1fr 1fr}
  .fd-row{grid-template-columns:1fr 1fr}
  .bt-head,.bt-row{grid-template-columns:2fr 1fr}
  .bt-head span:nth-child(3),.bt-head span:nth-child(4),.bt-row span:nth-child(3),.bt-row span:nth-child(4){display:none}
  .report-grid{grid-template-columns:1fr}.budget-grid{grid-template-columns:1fr 1fr}
  .stat-value{font-size:16px}.page-header h1{font-size:21px}
